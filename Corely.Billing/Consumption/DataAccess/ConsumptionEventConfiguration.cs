...
internal class ConsumptionEventConfiguration(IDbTypes dbTypes)
    : EntityConfigurationBase<ConsumptionEventEntity>(dbTypes)
{
    private const int OUTCOME_MAX_LENGTH = 20;
    ...
    protected override void ConfigureInternal(EntityTypeBuilder<ConsumptionEventEntity> builder)
    {
        ...
        builder.Property(e => e.Outcome).HasConversion<string>().HasMaxLength(OUTCOME_MAX_LENGTH);

        builder
            .Property(e => e.Provider)
            .HasMaxLength(ConsumptionEvent.PROVIDER_MAX_LENGTH)
            .IsRequired();

        builder
            .Property(e => e.IdempotencyKey)
            .HasMaxLength(ConsumptionEvent.IDEMPOTENCY_KEY_MAX_LENGTH)
            .IsRequired();
