...
This guide defines how documentation should be written for Corely libraries so that all projects in the suite (Corely.Common, Corely.Security, Corely.DataAccess, Corely.IAM, Corely.IAM.Web, Corely.Billing) feel like they belong together. Derived from analysis of the existing documentation across the three published Corely libraries.
...
- **Billing**: Options → Registration → Operation scope → Service → Result
