...
Command-line tool for managing the Corely.Billing database — creation, migration, scripting, and status. Built on `System.CommandLine` and distributed as a .NET tool.
...
The command is `corely-billing-db`. Running from a clone of this repository works too — substitute `dotnet run --project Corely.Billing.DataAccessMigrations.Cli --` for `corely-billing-db` in every example below.
...
```bash
corely-billing-db db migrate
```

Migrate to a specific migration (applies or reverts as needed):

```bash
corely-billing-db db migrate "MigrationName"
```

Revert all migrations:

```bash
corely-billing-db db migrate 0
```
...
```bash
corely-billing-db db script
```

Generate a script between two specific migrations:

```bash
corely-billing-db db script "FromMigration" "ToMigration"
```
...
```bash
corely-billing-db db script -i -o "deploy.sql"
```
...
```bash
corely-billing-db db test-connection
```

### Drop Database

```bash
corely-billing-db db drop
```
...
```bash
corely-billing-db provider list
```
...
| `MsSql` | `Corely.Billing.DataAccessMigrations.MsSql` |
...
```bash
corely-billing-db db create -p MsSql -c "your-connection-string"
```
