using Corely.Billing.DataAccessMigrations.Cli.Commands.DatabaseCommands;
...
public class DbCommandTests : IDisposable
{
    ...
    [Fact]
    public void Script_UsesBillingHistoryTable_ByDefault()
    {
        var output = CommandRunner.Run(new Script(), "--provider", "MsSql");

        Assert.Contains("__CorelyBillingMigrationsHistory", output);
    }
    ...
    public void Script_HonorsHistoryTableOverride()
    {
        ...
        Assert.DoesNotContain("__CorelyBillingMigrationsHistory", output);
